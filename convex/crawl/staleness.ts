...
async function deleteDocAndChunks(
	...
	for (const chunk of chunks) {
		try {
			...
			console.warn(
				`Failed to delete vector ${chunk.ragId} from RAG during purge:`,
				err,
			);
		}
	}
	...
	handler: async (ctx, { limit }) => {
		...
		for (const status of FLAGGED_STATUSES) {
			...
			const isStaleDocs = await ctx.db
				.query("documents")
				.withIndex("by_status_and_isStale", (q) =>
					q.eq("status", status).eq("isStale", true),
				)
...
export const flagExpiredDocuments = internalMutation({
	...
	handler: async (ctx, { limit }) => {
		...
		const indexedDocs = await ctx.db
			.query("documents")
			.withIndex("by_status_and_isStale", (q) =>
				q.eq("status", "indexed").eq("isStale", false),
			)
			.take(batchSize);
		...
		const activeDocs = await ctx.db
			.query("documents")
			.withIndex("by_status_and_isStale", (q) =>
				q.eq("status", "active").eq("isStale", false),
			)
			.take(batchSize);
