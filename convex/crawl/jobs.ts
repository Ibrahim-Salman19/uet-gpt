...
async function deleteResolvedDLQ(
	ctx: any,
	batchSize: number,
	cutoff: number,
): Promise<number> {
...
async function trimStaleDLQPayloads(
	ctx: any,
	batchSize: number,
	cutoff: number,
): Promise<number> {
...
async function compactOrphanedChunks(
	ctx: any,
	batchSize: number,
): Promise<number> {
		...
		if (!doc) {
			const children = await ctx.db
				...
				.withIndex("by_documentId", (q: any) =>
					q.eq("documentId", parent.documentId),
				)
				.take(batchSize);
			for (const child of children) {
				try {
					await rag.delete(ctx, {
						entryId:
							child.ragId as unknown as import("@convex-dev/rag").EntryId,
					});
				} catch (err) {
...
export const cleanupOldRecords = internalMutation({
	...
	handler: async (ctx, { limit }) => {
		...
		totalDeleted += await deleteAbandonedDLQ(
			ctx,
			batchSize,
			now,
			SEVEN_DAYS_MS,
		...
		if (totalDeleted < batchSize) {
			...
			totalDeleted += await deleteOldJobsByStatus(
				ctx,
				remaining,
				now,
				THIRTY_DAYS_MS,
