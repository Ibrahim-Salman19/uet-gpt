...
export function VoiceModal({ onTranscript }: VoiceModalProps) {
  ...
  // Open/close dialog and auto-start listening
  React.useEffect(() => {
  ...
  return (
    ...
    >
      <div className="bg-[#101012] border border-[#2d2d34] rounded-[1.5rem] shadow-[0_32px_64px_-12px_rgba(0,0,0,0.9)] p-6 flex flex-col items-center text-center gap-4">
        ...
        {/* Animated waveform — active when listening */}
            ...
            <div
              ...
              style={{
                ...
                background: isListening
                  ? "var(--accent)"
                  : transcript
                    ? "oklch(0.55 0.18 145)" // green when done
        ...
        {/* Error message */}
        {error && (
          <p className="text-xs text-red-400 font-sans -mt-2 px-2">{error}</p>
        )}
