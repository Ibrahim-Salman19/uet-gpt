...
export function Sidebar({ className }: SidebarProps) {
  ...
  const filteredChats = debouncedSearchQuery
    ? chatItems.filter((c) => c.title.toLowerCase().includes(debouncedSearchQuery.toLowerCase()))
    : chatItems;
  ...
  if (error) {
    return (
      <div className="flex-1 flex flex-col overflow-hidden items-center justify-center p-4 text-center">
        <svg className="w-8 h-8 text-red-500/80 mb-2" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          <circle cx="12" cy="12" r="10" />
          <line x1="12" y1="8" x2="12" y2="12" />
          <line x1="12" y1="16" x2="12.01" y2="16" />
        </svg>
        <span className="text-xs text-zinc-400 font-medium">{error}</span>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden">
      ...
      <div className="px-3 py-2 border-t border-white/[0.04]">
        <SidebarSearch value={searchQuery} onChange={handleSearchChange} />
      </div>

      {/* Scrollable history + pins */}
      ...
      <div className="p-3 border-t border-[#222226] flex items-center justify-between gap-2 shrink-0 bg-[#0a0a0c]/60">
        ...
        >
          <svg className="w-4 h-4 transition-colors" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
