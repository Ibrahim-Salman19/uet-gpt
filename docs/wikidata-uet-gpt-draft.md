...
> Note on **license (P275)**: Assert `AGPL-3.0` (Q38339) since the repository ships a LICENSE file.
...
> **Notability requirement:** Wikidata requires that a new item be *notable* - verifiable through independent, reliable published sources. The first-party references above (project site and own website are the same domain) are **not sufficient by themselves**. Before creating the item, secure at least one **independent third-party** reference (e.g., a news article, a university page mentioning UET GPT, a directory listing, or a blog post) that discusses UET GPT. See the Wikipedia section below for how to build toward that.
...
3. **Cite an independent source.** The sentence must be backed by a reliable, independent reference (not just the project site). Options:
...
5. **Once an independent source exists and the Wikipedia mention is live**, that Wikipedia article itself becomes a strong, independent Wikidata reference - satisfying the notability requirement from Section 3, and you can then safely create the Wikidata item with `described by source` pointing to both the project site and the Wikipedia article.
